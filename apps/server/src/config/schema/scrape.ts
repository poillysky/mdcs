...
function normalizeDownloadOverride(raw: unknown): KindDownloadOverride | undefined {
  ...
  if (typeof raw.subtitleLibraryPath === "string") {
    const sub = raw.subtitleLibraryPath.trim();
    out.subtitleLibraryPath = sub ? toStorageRelativePath(sub) : "";
  }
...
function assertScrapeConfig(config: ScrapeConfig): void {
  ...
  if (config.exportFastConcurrency < 1) {
    throw new Error("配置错误: exportFastConcurrency 必须大于 0");
  }
...
export function normalizeScrapeConfig(raw: unknown): ScrapeConfig {
  ...
  const config: ScrapeConfig = {
    ...
    requestTimeoutSec: Math.max(
      5,
      Math.min(120, Math.floor(toNumberOr(raw.requestTimeoutSec, base.requestTimeoutSec))),
    ),
    ...
    download: (() => {
      ...
      return {
        ...
        skipAmazon,
        subtitleLibraryPath: (() => {
          const sub = toStringOr(downloadRaw.subtitleLibraryPath, base.download.subtitleLibraryPath).trim();
          return sub ? toStorageRelativePath(sub) : "";
        })(),
