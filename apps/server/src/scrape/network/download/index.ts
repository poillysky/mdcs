import fs from "node:fs";
import path from "node:path";
import { spawn } from "node:child_process";
import os from "node:os";
import { fetch as undiciFetch } from "undici";
import {
  fetchViaFlareSolverrFull,
  getCachedClearance,
  getFlareSolverrUrl,
  hasFlareSession,
  hostNeedsFlare,
  looksBlockedHtml,
  mergeCookieHeaders,
  clearCachedClearance,
} from "../flaresolverr.js";
import { getActiveProxy } from "../proxy.js";
import { cookieForUrl } from "../sourceCookies.js";
import { UA } from "../ua.js";
import { isAdaptiveFlareSource } from "../adaptiveFlare.js";
import { PROJECT_ROOT } from "../../../paths.js";

import {
  MAX_HTML_BYTES,
  MAX_IMAGE_BYTES,
  clearDirectSkip,
  contentLengthTooLarge,
  drainBody,
  hostOf,
  markDirectSkip,
  shouldSkipDirect,
  withHostGate,
} from "./hostGate.js";


export async function downloadToFile(
  url: string,
  dest: string,
  opts?: { timeoutMs?: number },
): Promise<boolean> {
  const buf = await downloadBytes(url, opts);
  if (!buf) return false;
  fs.mkdirSync(path.dirname(dest), { recursive: true });
  const tmp = `${dest}.tmp`;
  fs.writeFileSync(tmp, buf);
  fs.renameSync(tmp, dest);
  return true;
}

/** 下载图片到内存（用于按文件大小择优） */
export async function downloadBytes(
  url: string,
  opts?: { timeoutMs?: number; maxBytes?: number },
): Promise<Buffer | null> {
  const timeoutMs = opts?.timeoutMs ?? 25000;
  const maxBytes = opts?.maxBytes ?? MAX_IMAGE_BYTES;
  if (!url || !/^https?:\/\//i.test(url)) return null;
  try {
    const res = await undiciFetch(url, {
      signal: AbortSignal.timeout(timeoutMs),
      headers: {
        "User-Agent": UA,
        Accept: "image/*,*/*;q=0.8",
        Referer: new URL(url).origin + "/",
      },
      redirect: "follow",
    });
    if (!res.ok) {
      await drainBody(res);
      return null;
    }
    if (contentLengthTooLarge(res.headers, maxBytes)) {
      console.log(
        `[scrape] image skip host=${hostOf(url)} Content-Length>${maxBytes}`,
      );
      await drainBody(res);
      return null;
    }
    const buf = Buffer.from(await res.arrayBuffer());
    if (buf.length < 200 || buf.length > maxBytes) return null;
    return buf;
  } catch {
    return null;
  }
}

async function fetchDirect(
  url: string,
  opts: {
    timeoutMs: number;
    referer?: string;
    cookie?: string;
    userAgent?: string;
    /** 带着 cf_clearance 试直连时失败：勿封 3 分钟直连（否则会逼所有请求重回慢速 FS） */
    hadClearance?: boolean;
  },
): Promise<{ html: string; finalUrl: string } | null> {
  try {
    const headers: Record<string, string> = {
      "User-Agent": opts.userAgent || UA,
      Accept: "text/html,application/xhtml+xml",
      "Accept-Language": "ja,zh-CN;q=0.9,zh;q=0.8,en;q=0.7",
    };
    if (opts.referer) headers.Referer = opts.referer;
    if (opts.cookie) headers.Cookie = opts.cookie;
    const res = await undiciFetch(url, {
      signal: AbortSignal.timeout(opts.timeoutMs),
      headers,
      redirect: "follow",
    });
    if (!res.ok) {
      if (
        (res.status === 403 || res.status === 503) &&
        !opts.hadClearance
      ) {
        markDirectSkip(hostOf(url));
      }
      console.log(
        `[scrape] cookie-direct fail host=${hostOf(url)} HTTP ${res.status}`,
      );
      await drainBody(res);
      return null;
    }
    if (contentLengthTooLarge(res.headers, MAX_HTML_BYTES)) {
      console.log(
        `[scrape] cookie-direct skip host=${hostOf(url)} Content-Length>${MAX_HTML_BYTES}`,
      );
      await drainBody(res);
      return null;
    }
    const buf = Buffer.from(await res.arrayBuffer());
    if (buf.length > MAX_HTML_BYTES) {
      console.log(
        `[scrape] cookie-direct skip host=${hostOf(url)} body>${MAX_HTML_BYTES}`,
      );
      return null;
    }
    const html = decodeHtmlBytes(buf, res.headers.get("content-type"));
    if (looksBlockedHtml(html)) {
      if (!opts.hadClearance) markDirectSkip(hostOf(url));
      console.log(
        `[scrape] cookie-direct fail host=${hostOf(url)} blocked/challenge ${html.length}b`,
      );
      return null;
    }
    return { html, finalUrl: String(res.url || url) };
  } catch (e) {
    console.log(
      `[scrape] cookie-direct fail host=${hostOf(url)} ${e instanceof Error ? e.message : "err"}`,
    );
    return null;
  }
}

type CurlBin = { bin: string; impersonate: boolean; extraArgs: string[] };

/** lexiforest v2 默认 Chrome 目标；可用 SCRAPE_CURL_IMPERSONATE 覆盖 */
function defaultChromeTarget(): string {
  const env = String(process.env.SCRAPE_CURL_IMPERSONATE || "").trim();
  return env || "chrome136";
}

/** Windows 的 .bat 包装脚本 Node spawn 会 EINVAL，改打 exe + --impersonate */
function normalizeCurlBin(bin: string): CurlBin {
  const impersonateHint = /chrome|impersonate|firefox|edge|safari/i.test(bin);
  const base = path.basename(bin);
  if (process.platform === "win32" && /\.(bat|cmd)$/i.test(base)) {
    const exe = path.join(path.dirname(bin), "curl-impersonate.exe");
    if (fs.existsSync(exe)) {
      const target = defaultChromeTarget();
      return {
        bin: exe,
        impersonate: true,
        extraArgs: ["--impersonate", target],
      };
    }
  }
  const extraArgs: string[] = [];
  const stem = base.replace(/\.(exe|bat|cmd)$/i, "").toLowerCase();
  if (stem === "curl-impersonate" || stem === "curl-impersonate-chrome") {
    extraArgs.push("--impersonate", defaultChromeTarget());
  }
  return {
    bin,
    impersonate: impersonateHint || extraArgs.length > 0,
    extraArgs,
  };
}

function bundledImpersonateExe(): string {
  return path.join(
    PROJECT_ROOT,
    "tools",
    "curl-impersonate",
    process.platform === "win32" ? "curl-impersonate.exe" : "curl-impersonate",
  );
}

/**
 * curl 回退：部分站（airav 等）用 TLS 指纹拦 Node/undici，同代理下 Python/curl 仍 200。
 * NAS 容器默认 OpenSSL curl 指纹易被 CF 拦；优先 curl-impersonate（Chrome TLS）。
 */
function resolveCurlBin(): CurlBin {
  const envBin = String(process.env.SCRAPE_CURL_BIN || "").trim();
  if (envBin) return normalizeCurlBin(envBin);

  const bundled = bundledImpersonateExe();
  if (fs.existsSync(bundled)) return normalizeCurlBin(bundled);

  if (process.platform === "win32") {
    return { bin: "curl.exe", impersonate: false, extraArgs: [] };
  }
  const candidates = [
    "curl_chrome146",
    "curl_chrome145",
    "curl_chrome142",
    "curl_chrome136",
    "curl_chrome131",
    "curl_chrome124",
    "curl_chrome116",
    "curl_chrome110",
    "curl-impersonate",
    "curl-impersonate-chrome",
    "curl",
  ];
  for (const name of candidates) {
    const abs = `/usr/local/bin/${name}`;
    if (fs.existsSync(abs)) return normalizeCurlBin(abs);
  }
  return { bin: "curl", impersonate: false, extraArgs: [] };
}

function curlLdEnv(): NodeJS.ProcessEnv {
  return {
    ...process.env,
    LD_LIBRARY_PATH: ["/usr/local/lib", String(process.env.LD_LIBRARY_PATH || "")]
      .filter(Boolean)
      .join(":"),
  };
}

let loggedCurlBin = false;

async function fetchViaCurl(
  url: string,
  opts: {
    timeoutMs: number;
    referer?: string;
    cookie?: string;
    userAgent?: string;
  },
): Promise<{ html: string; finalUrl: string } | null> {
  const { bin: curlBin, impersonate, extraArgs } = resolveCurlBin();
  if (!loggedCurlBin) {
    loggedCurlBin = true;
    console.log(
      `[scrape] curl-bin=${curlBin} impersonate=${impersonate ? "yes" : "no"}${extraArgs.length ? ` ${extraArgs.join(" ")}` : ""}`,
    );
  }
  const tmp = path.join(
    os.tmpdir(),
    `scrape-curl-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.body`,
  );
  const args: string[] = [
    ...extraArgs,
    "-sS",
    "-L",
    "--compressed",
    "--connect-timeout",
    String(Math.max(3, Math.min(12, Math.ceil(opts.timeoutMs / 1000)))),
    "--max-time",
    String(Math.max(5, Math.ceil(opts.timeoutMs / 1000))),
    "-H",
    "Accept: text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8",
    "-H",
    "Accept-Language: zh-CN,zh;q=0.9,en;q=0.8",
    "-o",
    tmp,
    "-w",
    "%{http_code}\n%{url_effective}",
  ];
  // impersonate 包装脚本自带 Chrome UA；乱盖 -A 会和 TLS 指纹不一致
  // clearance 复用时必须用当时的 UA
  if (opts.userAgent) {
    args.push("-A", opts.userAgent);
  } else if (!impersonate) {
    args.push("-A", UA);
  }
  const proxy = getActiveProxy();
  if (proxy) {
    args.push("-x", proxy);
  }
  if (opts.referer) {
    args.push("-e", opts.referer);
  }
  if (opts.cookie) {
    args.push("-H", `Cookie: ${opts.cookie}`);
  }
  args.push(url);

  try {
    const { code, stdout } = await new Promise<{
      code: number;
      stdout: string;
    }>((resolve) => {
      const child = spawn(curlBin, args, {
        windowsHide: true,
        stdio: ["ignore", "pipe", "pipe"],
        env: curlLdEnv(),
      });
      let out = "";
      let err = "";
      child.stdout.on("data", (c: Buffer) => {
        out += c.toString("utf8");
      });
      child.stderr.on("data", (c: Buffer) => {
        err += c.toString("utf8");
      });
      child.on("close", (c) => {
        if (err && !out) {
          console.log(
            `[scrape] curl stderr host=${hostOf(url)} ${err.slice(0, 160)}`,
          );
        }
        resolve({ code: c ?? 1, stdout: out });
      });
      child.on("error", (e) => {
        console.log(
          `[scrape] curl spawn fail bin=${curlBin} host=${hostOf(url)} ${e.message}`,
        );
        resolve({ code: 1, stdout: "" });
      });
    });

    const lines = stdout.trim().split(/\r?\n/);
    const httpCode = Number(lines[0] || 0);
    const finalUrl = String(lines[1] || url).trim() || url;
    if (code !== 0 || httpCode < 200 || httpCode >= 400) {
      console.log(
        `[scrape] curl fail bin=${path.basename(curlBin)} host=${hostOf(url)} exit=${code} HTTP ${httpCode || "-"}`,
      );
      return null;
    }
    if (!fs.existsSync(tmp)) return null;
    const st = fs.statSync(tmp);
    if (st.size > MAX_HTML_BYTES) {
      console.log(
        `[scrape] curl skip host=${hostOf(url)} body>${MAX_HTML_BYTES}`,
      );
      return null;
    }
    const buf = fs.readFileSync(tmp);
    if (buf.length < 400) return null;
    const html = decodeHtmlBytes(buf, null);
    if (looksBlockedHtml(html)) {
      console.log(
        `[scrape] curl blocked bin=${path.basename(curlBin)} host=${hostOf(url)} ${html.length}b`,
      );
      return null;
    }
    console.log(
      `[scrape] curl-ok bin=${path.basename(curlBin)} host=${hostOf(url)} ${html.length}b proxy=${proxy ? "on" : "off"}`,
    );
    return { html, finalUrl };
  } catch (e) {
    console.log(
      `[scrape] curl err host=${hostOf(url)} ${e instanceof Error ? e.message : "err"}`,
    );
    return null;
  } finally {
    try {
      if (fs.existsSync(tmp)) fs.unlinkSync(tmp);
    } catch {
      /* ignore */
    }
  }
}

function looksLikeImageBytes(buf: Buffer): boolean {
  if (buf.length < 12) return false;
  if (buf[0] === 0xff && buf[1] === 0xd8 && buf[2] === 0xff) return true;
  if (buf.subarray(0, 8).equals(Buffer.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]))) {
    return true;
  }
  if (buf.subarray(0, 4).toString("ascii") === "RIFF" && buf.subarray(8, 12).toString("ascii") === "WEBP") {
    return true;
  }
  return false;
}

/** 二进制下载（封面 CDN）；对齐 MDCX：Flare 拿 Cookie/UA 后 curl 带 Referer 拉图 */
export async function fetchBinaryViaCurl(
  url: string,
  opts: {
    timeoutMs: number;
    referer?: string;
    cookie?: string;
    userAgent?: string;
    /** MDCX jdbstatic：Sec-Fetch-Dest/Mode/Site */
    secFetchImage?: boolean;
    /** true=不走全局代理（CDN 直连） */
    direct?: boolean;
  },
): Promise<Buffer | null> {
  const { bin: curlBin, impersonate, extraArgs } = resolveCurlBin();
  const tmp = path.join(
    os.tmpdir(),
    `scrape-curl-bin-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.body`,
  );
  const args: string[] = [
    ...extraArgs,
    "-sS",
    "-L",
    "--compressed",
    "--connect-timeout",
    String(Math.max(3, Math.min(12, Math.ceil(opts.timeoutMs / 1000)))),
    "--max-time",
    String(Math.max(5, Math.ceil(opts.timeoutMs / 1000))),
    "-H",
    "Accept: image/avif,image/webp,image/apng,image/svg+xml,image/*,*/*;q=0.8",
    "-H",
    "Accept-Language: zh-CN,zh;q=0.9,en;q=0.8",
    "-o",
    tmp,
    "-w",
    "%{http_code}",
  ];
  if (opts.userAgent) {
    args.push("-A", opts.userAgent);
  } else if (!impersonate) {
    args.push("-A", UA);
  }
  const proxy = opts.direct ? "" : getActiveProxy();
  if (proxy) args.push("-x", proxy);
  if (opts.referer) args.push("-e", opts.referer);
  if (opts.cookie) args.push("-H", `Cookie: ${opts.cookie}`);
  if (opts.secFetchImage) {
    args.push("-H", "Sec-Fetch-Dest: image");
    args.push("-H", "Sec-Fetch-Mode: no-cors");
    args.push("-H", "Sec-Fetch-Site: cross-site");
  }
  args.push(url);

  try {
    const { code, stdout } = await new Promise<{ code: number; stdout: string }>((resolve) => {
      const child = spawn(curlBin, args, {
        windowsHide: true,
        stdio: ["ignore", "pipe", "pipe"],
        env: curlLdEnv(),
      });
      let out = "";
      child.stdout.on("data", (c: Buffer) => {
        out += c.toString("utf8");
      });
      child.on("close", (c) => resolve({ code: c ?? 1, stdout: out }));
      child.on("error", () => resolve({ code: 1, stdout: "" }));
    });
    const httpCode = Number(String(stdout).trim() || 0);
    if (code !== 0 || httpCode < 200 || httpCode >= 400) {
      console.log(
        `[scrape] curl-bin fail host=${hostOf(url)} exit=${code} HTTP ${httpCode || "-"}`,
      );
      return null;
    }
    if (!fs.existsSync(tmp)) return null;
    const st = fs.statSync(tmp);
    if (st.size <= 0 || st.size > MAX_IMAGE_BYTES) return null;
    const buf = fs.readFileSync(tmp);
    if (!looksLikeImageBytes(buf)) {
      console.log(`[scrape] curl-bin not-image host=${hostOf(url)} ${buf.length}b`);
      return null;
    }
    console.log(`[scrape] curl-bin ok host=${hostOf(url)} ${buf.length}b${opts.direct ? " direct" : ""}`);
    return buf;
  } catch {
    return null;
  } finally {
    try {
      if (fs.existsSync(tmp)) fs.unlinkSync(tmp);
    } catch {
      /* ignore */
    }
  }
}

/** 按 Content-Type / meta charset 解码（カリビアンコム等为 EUC-JP） */
function decodeHtmlBytes(
  buf: Buffer,
  contentType: string | null | undefined,
): string {
  const ct = String(contentType || "");
  let charset =
    ct.match(/charset\s*=\s*["']?([^\s;"']+)/i)?.[1]?.trim() || "";
  if (!charset) {
    const head = buf.subarray(0, Math.min(buf.length, 8192)).toString("latin1");
    charset =
      head.match(
        /<meta[^>]+charset\s*=\s*["']?([^\s"'/>]+)/i,
      )?.[1]?.trim() ||
      head.match(
        /content=["'][^"']*charset\s*=\s*([^\s"';]+)/i,
      )?.[1]?.trim() ||
      "";
  }
  const key = charset.toLowerCase().replace(/_/g, "-");
  const enc =
    key === "euc-jp" || key === "eucjp" || key === "x-euc-jp"
      ? "euc-jp"
      : key === "shift-jis" ||
          key === "shiftjis" ||
          key === "sjis" ||
          key === "windows-31j" ||
          key === "cp932"
        ? "shift_jis"
        : key === "iso-2022-jp"
          ? "iso-2022-jp"
          : "utf-8";
  try {
    return new TextDecoder(enc).decode(buf);
  } catch {
    try {
      return buf.toString("utf8");
    } catch {
      return buf.toString("latin1");
    }
  }
}

export type FetchPageVia = "direct" | "curl" | "flare";

export type FetchPageResult = {
  html: string;
  finalUrl: string;
  /** 实际取页通道：代理/直连 Node · curl · FlareSolverr */
  via?: FetchPageVia;
};

/**
 * 抓取页面并带回最终落地 URL（用于识别镜像跳转）。
 * 同 host 串行：第一个请求完成并写入 clearance 后，后续才复用，避免并发同时过盾。
 */
export async function fetchPage(
  url: string,
  opts?: {
    timeoutMs?: number;
    referer?: string;
    cookie?: string;
    userAgent?: string;
    sourceId?: string;
    access?: string;
    viaFlare?: boolean;
    strictTimeout?: boolean;
    /** SPA 站内跳转后等渲染（传给 FlareSolverr） */
    waitInSeconds?: number;
    /** true=禁止 sessions.create（测通） */
    noSessionRetry?: boolean;
    /** 测通：忽略 clearance/session，避免过期凭证导致 skip-curl */
    freshProbe?: boolean;
  },
): Promise<FetchPageResult | null> {
  const host = hostOf(url);
  return withHostGate(host, () => fetchPageUnlocked(url, opts));
}

async function fetchPageUnlocked(
  url: string,
  opts?: {
    timeoutMs?: number;
    referer?: string;
    cookie?: string;
    userAgent?: string;
    sourceId?: string;
    access?: string;
    viaFlare?: boolean;
    strictTimeout?: boolean;
    waitInSeconds?: number;
    noSessionRetry?: boolean;
    freshProbe?: boolean;
  },
): Promise<FetchPageResult | null> {
  const timeoutMs = opts?.timeoutMs ?? 20000;
  const freshProbe = Boolean(opts?.freshProbe);
  if (freshProbe) clearDirectSkip(hostOf(url));
  const flareTimeoutMs = opts?.strictTimeout
    ? timeoutMs
    : Math.max(timeoutMs, freshProbe ? 35000 : 45000);
  const baseCookie =
    String(opts?.cookie || "").trim() || cookieForUrl(url, opts?.sourceId);
  const adaptive =
    opts?.access === "proxy_adaptive" ||
    opts?.access === "proxy" ||
    isAdaptiveFlareSource(opts?.sourceId);
  // adaptive：允许过盾回落（即使调用方写了 viaFlare:false）
  const flareOn =
    Boolean(getFlareSolverrUrl()) &&
    (adaptive || opts?.viaFlare !== false);
  const waitInSeconds = opts?.waitInSeconds;
  const spaWait = Boolean(waitInSeconds && waitInSeconds > 0);
  // 测通：勿用磁盘 clearance / 活跃 session，否则 skip-curl 后 Flare 空响
  const cached = freshProbe ? null : getCachedClearance(url);
  const liveSession = freshProbe ? false : flareOn && hasFlareSession(url);
  const cookie = mergeCookieHeaders(baseCookie, cached?.cookieHeader);
  const userAgent =
    String(opts?.userAgent || "").trim() || cached?.userAgent || undefined;
  const host = hostOf(url);

  // viaFlare:true 冷启动可跳过 Node 直连；已有凭证时改走 impersonate curl，勿空烧 undici
  const hasClearance = Boolean(cached?.cookieHeader);
  const skipDirectFirst =
    !freshProbe &&
    !adaptive &&
    ((flareOn && opts?.viaFlare === true && !hasClearance) ||
      (flareOn && shouldSkipDirect(host) && !hasClearance));

  const { impersonate } = resolveCurlBin();
  const minCurlBytes = waitInSeconds && waitInSeconds > 0 ? 2000 : 500;
  const curlOpts = {
    timeoutMs: Math.max(timeoutMs, hasClearance ? 12000 : 15000),
    referer: opts?.referer,
    cookie,
    userAgent,
  };
  const acceptCurl = (page: { html: string; finalUrl: string } | null) =>
    Boolean(
      page &&
        page.html.length >= minCurlBytes &&
        !looksBlockedHtml(page.html),
    );
  let curlTried = false;

  // 全源优先 impersonate curl（含 proxy_flare）；SPA wait 时 curl 只能拿空壳，直接过盾
  if (impersonate && !spaWait) {
    curlTried = true;
    if (hasClearance) {
      console.log(
        `[scrape] clearance-curl host=${host} (impersonate + cf_clearance)`,
      );
    } else {
      console.log(`[scrape] curl-first host=${host} impersonate`);
    }
    let viaCurl = await fetchViaCurl(url, curlOpts);
    if (acceptCurl(viaCurl)) {
      clearDirectSkip(host);
      if (hasClearance) {
        console.log(
          `[scrape] clearance-curl-ok host=${host} ${viaCurl!.html.length}b`,
        );
      }
      return { ...viaCurl!, via: "curl" };
    }
    if (!hasClearance) {
      await new Promise((r) => setTimeout(r, 800));
      viaCurl = await fetchViaCurl(url, curlOpts);
      if (acceptCurl(viaCurl)) {
        clearDirectSkip(host);
        console.log(
          `[scrape] curl-retry-ok host=${host} ${viaCurl!.html.length}b`,
        );
        return { ...viaCurl!, via: "curl" };
      }
    } else {
      console.log(`[scrape] clearance-curl miss host=${host} → next`);
    }
  } else if (spaWait && flareOn) {
    console.log(`[scrape] spa-wait host=${host} skip-curl → flare`);
  } else if (adaptive && opts?.viaFlare !== true && !hasClearance && !liveSession) {
    curlTried = true;
    let viaCurl = await fetchViaCurl(url, curlOpts);
    if (acceptCurl(viaCurl)) {
      clearDirectSkip(host);
      return { ...viaCurl!, via: "curl" };
    }
    await new Promise((r) => setTimeout(r, 800));
    viaCurl = await fetchViaCurl(url, curlOpts);
    if (acceptCurl(viaCurl)) {
      clearDirectSkip(host);
      console.log(`[scrape] curl-retry-ok host=${host} ${viaCurl!.html.length}b`);
      return { ...viaCurl!, via: "curl" };
    }
  } else if (hasClearance || liveSession) {
    console.log(
      `[scrape] clearance-reuse host=${host} skip-curl → flare (session=${liveSession ? "yes" : "no"}; need curl-impersonate for cookie+curl)`,
    );
  }

  // 有 Flare 凭证时 undici 也对不上 Chrome TLS；SPA wait 时直连同样只有空壳
  if (!skipDirectFirst && !hasClearance && !liveSession && !spaWait) {
    // adaptive：Node 只短探，把时间留给上面的 curl / 后面的 Flare（避免 NAS 上 40s+ 空耗）
    const directTimeout = adaptive
      ? Math.min(timeoutMs, hasClearance ? 12000 : 8000)
      : hasClearance
        ? Math.max(timeoutMs, 12000)
        : getActiveProxy() && hostNeedsFlare(url)
          ? Math.max(timeoutMs, 15000)
          : liveSession
            ? Math.min(timeoutMs, 8000)
            : timeoutMs;
    const direct = await fetchDirect(url, {
      timeoutMs: directTimeout,
      referer: opts?.referer,
      cookie,
      userAgent,
      hadClearance: hasClearance,
    });
    // SPA 源要求 wait 时，直连结果太短/空壳则视为未渲染，回退 FS
    const minBytes = waitInSeconds && waitInSeconds > 0 ? 2000 : 500;
    if (direct && direct.html.length >= minBytes && !looksBlockedHtml(direct.html)) {
      // 直连恢复：清掉该 host 的短时 skip，避免后续误伤
      clearDirectSkip(host);
      console.log(
        `[scrape] cookie-direct host=${host} ${direct.html.length}b proxy=${getActiveProxy() ? "on" : "off"} clearance=${hasClearance ? "yes" : "no"}${adaptive ? " adaptive" : ""}`,
      );
      return { ...direct, via: "direct" };
    }
    if (direct && (direct.html.length < minBytes || looksBlockedHtml(direct.html))) {
      console.log(
        `[scrape] cookie-direct ${looksBlockedHtml(direct.html) ? "blocked" : "thin"} host=${host} ${direct.html.length}b → ${adaptive || flareOn ? "flare" : "fail"}`,
      );
      if (adaptive && direct.html.length >= minBytes && looksBlockedHtml(direct.html)) {
        markDirectSkip(host);
      }
    }
    // clearance 勿因 undici/curl 失败就清：凭证来自 Flare Chrome，TLS 对不上≠ cookie 失效
    // 仅当直连拿到「挑战页」且已带 clearance 时，才视为过期
    if (
      hasClearance &&
      direct &&
      looksBlockedHtml(direct.html) &&
      direct.html.length >= 500
    ) {
      clearCachedClearance(url);
    }
  }

  // Node/undici 易被 CF TLS 指纹拦（同代理 Python/curl 仍 200）：viaFlare:false 时用 curl 回退
  // 显式 false 且非 adaptive：禁止再回落 Flare
  if (opts?.viaFlare === false && !adaptive) {
    if (!curlTried) {
      const viaCurl = await fetchViaCurl(url, {
        timeoutMs,
        referer: opts?.referer,
        cookie,
        userAgent,
      });
      if (viaCurl) {
        clearDirectSkip(host);
        return { ...viaCurl, via: "curl" };
      }
    }
    return null;
  }

  if (
    flareOn &&
    (adaptive ||
      opts?.viaFlare === true ||
      hostNeedsFlare(url) ||
      hasClearance ||
      liveSession ||
      skipDirectFirst)
  ) {
    try {
      // 上 Flare 前再试一次 curl（已 impersonate 试过则跳过）
      if (
        !curlTried &&
        !spaWait &&
        adaptive &&
        opts?.viaFlare !== true &&
        !hasClearance &&
        !liveSession
      ) {
        await new Promise((r) => setTimeout(r, 600));
        const preFlareCurl = await fetchViaCurl(url, {
          timeoutMs: Math.max(timeoutMs, 12000),
          referer: opts?.referer,
          cookie,
          userAgent,
        });
        if (
          preFlareCurl &&
          preFlareCurl.html.length >= 500 &&
          !looksBlockedHtml(preFlareCurl.html)
        ) {
          clearDirectSkip(host);
          console.log(
            `[scrape] curl-pre-flare-ok host=${host} ${preFlareCurl.html.length}b`,
          );
          return { ...preFlareCurl, via: "curl" };
        }
      }
      if (adaptive) {
        console.log(
          `[scrape] adaptive-flare host=${host}${hasClearance ? " +clearance" : ""}`,
        );
      }
      // 把 clearance 也带进 FS，session 重建时少打一轮挑战
      const hit = await fetchViaFlareSolverrFull(url, {
        timeoutMs: flareTimeoutMs,
        cookie: cookie || baseCookie || undefined,
        waitInSeconds,
        // 测通显式 noSessionRetry；否则 strictTimeout 也禁止 create
        noSessionRetry: opts?.noSessionRetry === true || opts?.strictTimeout === true,
      });
      if (hit.html && !looksBlockedHtml(hit.html)) {
        return {
          html: hit.html,
          finalUrl: hit.finalUrl || url,
          via: "flare",
        };
      }
    } catch {
      /* fall through */
    }
  }
  return null;
}

export async function fetchText(
  url: string,
  opts?: {
    timeoutMs?: number;
    referer?: string;
    cookie?: string;
    sourceId?: string;
    /** true=强制走 FS；false=禁用；默认遇挑战再过盾 */
    viaFlare?: boolean;
    /** true=过盾也遵守 timeoutMs，不抬到 45s（连通性探测） */
    strictTimeout?: boolean;
    waitInSeconds?: number;
  },
): Promise<string | null> {
  const page = await fetchPage(url, opts);
  return page?.html || null;
}

/** JSON API（LibreDMM 等）；走全局代理，不过盾 */
export async function fetchJson(
  url: string,
  opts?: { timeoutMs?: number; referer?: string },
): Promise<unknown | null> {
  const timeoutMs = opts?.timeoutMs ?? 20000;
  try {
    const headers: Record<string, string> = {
      "User-Agent": UA,
      Accept: "application/json",
      "Accept-Language": "ja,en-US;q=0.8,en;q=0.6",
    };
    if (opts?.referer) headers.Referer = opts.referer;
    const res = await undiciFetch(url, {
      signal: AbortSignal.timeout(timeoutMs),
      headers,
      redirect: "follow",
    });
    if (!res.ok && res.status !== 202) return null;
    const text = await res.text();
    if (!text) return null;
    const ct = String(res.headers.get("content-type") || "").toLowerCase();
    // JSON 短包（err=processing / not_found）勿当 CF 壳页丢掉
    if (!ct.includes("json") && looksBlockedHtml(text)) return null;
    if (ct.includes("html") || /^\s*</.test(text)) return null;
    return JSON.parse(text) as unknown;
  } catch {
    return null;
  }
}

export async function fetchPostForm(
  url: string,
  body: string,
  opts?: {
    timeoutMs?: number;
    referer?: string;
    cookie?: string;
    sourceId?: string;
  },
): Promise<string | null> {
  const timeoutMs = opts?.timeoutMs ?? 15000;
  const cookie =
    String(opts?.cookie || "").trim() || cookieForUrl(url, opts?.sourceId);
  try {
    const headers: Record<string, string> = {
      "User-Agent": UA,
      "Content-Type": "application/x-www-form-urlencoded",
      Accept: "text/html",
      Referer: opts?.referer || new URL(url).origin + "/",
    };
    if (cookie) headers.Cookie = cookie;
    const res = await undiciFetch(url, {
      method: "POST",
      signal: AbortSignal.timeout(timeoutMs),
      headers,
      body,
      redirect: "follow",
    });
    if (!res.ok) return null;
    const html = await res.text();
    if (looksBlockedHtml(html)) return null;
    return html;
  } catch {
    return null;
  }
}

/** HEAD/range 探测图片是否可用（DMM 占位图过滤） */
export async function probeImageUrl(
  url: string,
  opts?: { timeoutMs?: number; referer?: string },
): Promise<{ ok: boolean; finalUrl: string; sizeHint: number }> {
  const timeoutMs = opts?.timeoutMs ?? 8000;
  try {
    const res = await undiciFetch(url, {
      method: "GET",
      signal: AbortSignal.timeout(timeoutMs),
      headers: {
        "User-Agent": UA,
        Referer: opts?.referer || new URL(url).origin + "/",
        Range: "bytes=0-2047",
      },
      redirect: "follow",
    });
    if (!res.ok && res.status !== 206) {
      await drainBody(res);
      return { ok: false, finalUrl: url, sizeHint: 0 };
    }
    const finalUrl = String(res.url || url);
    const cl = Number(res.headers.get("content-length") || 0);
    const rangeTotal = Number(
      (res.headers.get("content-range") || "").split("/")[1] || 0,
    );
    await drainBody(res);
    return { ok: true, finalUrl, sizeHint: rangeTotal || cl };
  } catch {
    return { ok: false, finalUrl: url, sizeHint: 0 };
  }
}
