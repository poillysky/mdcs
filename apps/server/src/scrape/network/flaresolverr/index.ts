...
import { looksBlockedHtml } from "./blocked.js";

export { looksBlockedHtml } from "./blocked.js";
 ...
 */
const CF_HOST_RE =
  /(?:^|\.)(javdb\.|javlibrary\.|missav|javten\.|fd2ppv\.|avmoo\.|avsox\.|avheat\.|tellme\.pw)/i;
...
async function destroySession(flare: string, sessionId: string): Promise<void> {
  ...
  if (!id) return;
  try {
    // destroy 卡住时绝不能拖 10s——会把整条过盾队列堵死
    await flareApi(
      flare,
      { cmd: "sessions.destroy", session: id },
      2500,
    );
    console.log(`[scrape] flare session destroy ${id.slice(0, 8)}…`);
  } catch (e) {
    console.warn(
      `[scrape] flare session destroy fail ${id.slice(0, 8)}…`,
      e instanceof Error ? e.message : e,
    );
  }
}

/** 开新会话前尽量清孤儿；单个 destroy 已短超时，失败直接跳过 */
async function destroyOrphanSessions(
  flare: string,
  keepId?: string | null,
): Promise<number> {
  let remote: string[] = [];
...
async function ensureHostSession(
  ...
  // 仅清「非本进程」孤儿；不要在每次 create 前死磕 destroy（远端 Chrome 挂死时会堵死）
  try {
    const remote = await listFlareSessions();
    const orphans = remote.filter((id) => id !== hit?.id);
    if (orphans.length >= 2) {
      // 堆积明显时才清；1 个可能是并发探测，勿折腾
      const n = await destroyOrphanSessions(flare, null);
      if (n > 0) console.log(`[scrape] flare purged orphan sessions=${n}`);
    }
  } catch {
    /* list 失败则直接 create */
  }
...
export function applyFlareSolverr(url: string | null | undefined): void {
  ...
  const prev = activeFlareUrl;
  const changed = next !== prev;
  activeFlareUrl = next;
  // 仅「从一个已配置 FS 切到另一个」时清 Cookie；冷启动 ""→url 不得清掉刚 load 的 cf_clearance
  if (changed && prev) {
    clearCachedClearance();
    // 旧 FS 上的 session 必须销毁，否则 Chrome 残留卡死
    const old = sharedSession;
    sharedSession = null;
    sessionByHost.clear();
    if (old?.id) {
      void destroySession(prev, old.id);
    }
  } else if (changed) {
    // 冷启动挂上 FS：只清 session 映射，保留磁盘 clearance
    sharedSession = null;
    sessionByHost.clear();
  }
  if (activeFlareUrl) {
...
/** CF / 边缘封锁 / 空壳页 / 站方封 IP — 见 ./blocked.ts */
...
/** 进程快退时立即落盘，避免 250ms debounce 丢 cf_clearance */
function flushPersistClearance(): void {
  if (persistTimer) {
...
function rememberClearance(
  ...
  // 立即落盘：短脚本/E2E 常在 debounce 前退出
  flushPersistClearance();
}
...
async function fetchViaFlareSolverrUnlocked(
  ...
  // AIO/Quasar SPA（waitInSeconds）：curl/复用 session 均无效，且广告 iframe 易搞崩 Chrome
  if (waitInSeconds > 0) {
    console.log(`[scrape] flare spa-wait host=${host} skip-session`);
    return await runOnce(null, false);
  }
  ...
  try {
    ...
    try {
      ...
      // session 失效：丢掉本地 + 异步 destroy（短超时），再无 session 直打
      // 对齐色花「不遗弃 Chrome」；但不连环 create+destroy 堵死队列
      const dead = sharedSession?.id || ensured.session.id;
      sharedSession = null;
      sessionByHost.clear();
      if (dead) void destroySession(flare, dead);
      console.warn(
        `[scrape] flare session request fail host=${host} → destroy+no-session`,
        e instanceof Error ? e.message : e,
      );
      return await runOnce(null, false);
    }
...
/** 进程退出 / 脚本结束：关掉本进程持有的 shared Chrome */
export async function releaseFlareSession(reason = "release"): Promise<void> {
  const hit = sharedSession;
  if (!hit) return;
  sharedSession = null;
  sessionByHost.clear();
  const flare = getFlareSolverrUrl();
  if (!flare) return;
  console.log(`[scrape] flare session release (${reason}) ${hit.id.slice(0, 8)}…`);
  await destroySession(flare, hit.id);
}

let exitHookInstalled = false;
export function installFlareExitHook(): void {
  if (exitHookInstalled || typeof process === "undefined") return;
  ...
  const shutdown = (sig: string) => {
    ...
    const flare = getFlareSolverrUrl();
    if (!flare) return;
    // exit 钩子里只能同步 fire-and-forget；尽量发 destroy
    console.log(`[scrape] flare session exit-hook (${sig}) ${hit.id.slice(0, 8)}…`);
    void destroySession(flare, hit.id);
  };
