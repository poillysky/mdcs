...
> **2026-08-27**：侧栏另有「七区任务」`/kind-tasks`；宽屏模式已落地（`localStorage mdcs.wideMode`）。暗色「切换主题」与 [UI-DESIGN-SYSTEM](./UI-DESIGN-SYSTEM.md) 浅色契约冲突，**不做**。
...
| 演员管理 | 完整 Emby 联动 | **已落地**：本地档案刮削 + Emby 同步（设置·演员） |
...
| 七区任务 | `/kind-tasks` | 按 Kind 快捷扫/跑 |
| 刮削记录 | `/records` | 历史记录列表 + 详情 |
| 演员管理 | `/actors` | 本地档案刮削、Emby 同步结果 |
| 文件管理 | `/files` | 扫描模式 / 文件树模式 |
| 数据源 | `/sources` | Provider + 优先级 |
| 设置 | `/settings/:tab` | 整理…Webhook 等 Tab |
...
本地档案 + Emby 真同步已落地（`ActorsSettingsPanel` / `ops.actors`）。
...
参考 MDC §18.4。本地 Tab + Emby Tab；支持搜索、状态筛选、刮削选中/缺失、详情档案。

| 控件 | 说明 |
|------|------|
| 搜索 | 按演员名 |
| 任务状态 | 已刮削 / 未刮削（本地）；成功/失败/跳过（Emby） |
| 表格列 | 头像、状态、简介摘要、时间、操作 |
| 空状态 | 「暂无数据」 |
